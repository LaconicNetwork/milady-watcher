...
import MiladysArtifacts from './artifacts/Miladys.json';
...
export class Indexer implements IndexerInterface {
  ...
  constructor (
    ...
    const { abi: MiladysABI, storageLayout: MiladysStorageLayout } = MiladysArtifacts;

    assert(MiladysABI);
    this._abiMap.set(KIND_MILADYS, MiladysABI);

    const MiladysContractInterface = new ethers.utils.Interface(MiladysABI);
    this._contractMap.set(KIND_MILADYS, MiladysContractInterface);

    const MiladysEventSignatures = Object.values(MiladysContractInterface.events).map(value => {
      return MiladysContractInterface.getEventTopic(value);
    });
    this.eventSignaturesMap.set(KIND_MILADYS, MiladysEventSignatures);

    assert(MiladysStorageLayout);
    this._storageLayoutMap.set(KIND_MILADYS, MiladysStorageLayout);
  }
